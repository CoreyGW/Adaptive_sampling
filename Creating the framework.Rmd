...
make_clusters<-function(grid_size=20, nclusters=3, avg_size=20, force_inclusion=FALSE){
  ...
  if(force_inclusion==T){
    ...
    locations<-locations%>%
      ...
      mutate(X=ifelse(Y>grid_size,grid_size,Y))%>%
      mutate(X=ifelse(Y<0, 0, Y))
...
```{r}
simulate_one<-function(nclusters=3, grid_size=20,n1=10,... ){
  # generate clusters
...
simulate_m<-function(m=10,nclusters=3, grid_size=20,n1=10,... ){
  results<-list()
  # generate clusters
  points<-make_clusters(...)
  for(i in 1:m){ # loop for the m samples
    ...
    neighbours<-apply(sample_tiles[occupied,], 1, function(x) get_neighbours(x,...))%>% # get neighbours
      bind_rows() %>% 
      as.data.frame() 
...
## Simulations with a soft border

### each sample is from a different population
```{r eval=FALSE}
n_sim<-1000
...
write.csv(results,file="Adaptive_Clustering_soft_border.csv")
```


### plot simulation results
```{r}
results<-read.csv("Adaptive_Clustering_soft_border.csv",show_col_types = FALSE)
...
### 1000 samples from the same population for 9 different populations
```{r eval=FALSE}
set.seed(pi)
n_samples<-1000
n_pops<-9
...
for(i in 1:n_pops){
  sim_data<-simulate_m(m=n_samples)
...
population_points
write.csv(results,file="sampling_same_pop.csv")
write.csv(population_points,file="points_for_populations.csv")
```
...
## Simulation with forced inclusion
### Inclusion forced in point generation
### only consider points INSIDE the grid
### 1000 samples from the same population for 9 different populations
```{r eval=FALSE}
set.seed(pi)
n_samples<-1000
